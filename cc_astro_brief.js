...
async function genAstroBrief(){
  ...
  try{
    const resp=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+key},body:JSON.stringify({model:'openai/gpt-oss-120b',messages:[{role:'user',content:prompt}],max_tokens:700,temperature:.7,reasoning_format:'hidden',reasoning_effort:'low'})});
    if(!resp.ok)return;
